...
```{r}
# UNIQUE FOR THIS SCRIPT - Select only COMIDs with TP samples (drop TN only rows). Also at this point we lose bad COMID for Twentymile Cr 12266-ORDEQ.
...
tp.cal <- tp_vars_selected %>%  
  filter(cal_val == 'CAL') %>%  
  select(-c(MLocID:COMID, logTP, SampleStartDate)) #drop logTP in favor of TP. Drop AWQMS columns too.
