...
bugsOTU=subset(bugsOTU, sampleId %in% rownames(preds))
        # SLH 11.9.22 - no records removed from bugs
...
rfdat<-data.frame(predcal,Groups=grps.11);
windows()
m=sf::st_as_sf(rfdat, coords = c("longitude", "latitude"), crs = 4269,agr = "constant")
mapview::mapview(m['Groups'],col.regions=brewer.pal(11, "Spectral"))
...
rfdat<-data.frame(predcal,grps.7, grps.8, grps.9, grps.10, grps.11);


rfdat <- rfdat %>%
  dplyr::select(Tmean8110Ws,Precip8110Ws,CaOWs,SLOPE,WsAreaSqKm,perStrm,SiO2Ws,PermWs,
         Fe2O3Ws,ElevCat,Eco2,Eco3,Eco4_code,East.West,grps.7,grps.8,grps.9,grps.10,grps.11)
...
rf.250ct_8grps = randomForest(grps.8 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat, #+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)
rf.250ct_9grps = randomForest(grps.9 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat, #+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)
rf.250ct_10grps = randomForest(grps.10 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat, #+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)
rf.250ct_11grps = randomForest(grps.11 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat, #+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)
...
varImpPlot(rf.200ct_7grps, type=1,n.var=10);
varImpPlot(rf.200ct_8grps, type=1,n.var=10);
varImpPlot(rf.200ct_9grps, type=1,n.var=10);
varImpPlot(rf.200ct_10grps, type=1,n.var=10);
varImpPlot(rf.200ct_11grps, type=1,n.var=10);
...
# 4.3) Specify the final predictor variables ;
      preds.final=c('Tmean8110Ws' , 'ElevCat', 'Eco3') # ,'Eco2','Eco3','Eco4_code','East.West'
...
#detach("package:NAMCr", unload = TRUE)
     save(bugcal.pa, predcal, grps.final, preds.final, rf.mod.best.from.VSURF11_250, file=('bugs analyses/RIVPACS_2022/rf.mod.best.from.VSURF11_250.Rdata')); # Notice I'm using the most parsimonious model (with 8 predictors)
...
load("bugs analyses/RIVPACS_2022/rf.mod.best.from.VSURF11_250.Rdata")
...
OE.assess.cal<-model.predict.RanFor.4.2(bugcal.pa,grps.final,preds.final, ranfor.mod=rf.mod.best.from.VSURF11_250,prednew=predcal,bugnew=bugcal.pa,Pc=0.5, Cal.OOB=TRUE); 
