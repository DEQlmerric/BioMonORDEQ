...
mapview::mapview(m['Groups'],col.regions=brewer.pal(2, 'Set1'), label=TRUE, use.layer.names=TRUE)
...
rfdat<-data.frame(predcal,grps.2, grps.3); #, grps.9, grps.10, grps.11


rfdat <- rfdat %>%
  dplyr::select(Tmean8110Ws,Precip8110Ws,CaOWs,SLOPE,WsAreaSqKm,perStrm,SiO2Ws,PermWs,
         Fe2O3Ws,ElevCat,Eco2,Eco3,Eco4_code,East.West,Lab, grps.2,grps.3)#,grps.9,grps.10,grps.11
...
rfdat$grps.3<-factor(rfdat$grps.3)
...
rf.250ct_2grps = randomForest(grps.2 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat, #,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)


rf.250ct_2grps = randomForest(grps.2 ~ Tmean8110Ws+SLOPE+WsAreaSqKm+perStrm, #,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)


rf.250ct_3grps = randomForest(grps.3 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat, #+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)



print(rf.250ct_2grps)

# Variable Importance Plot (VIP):

varImpPlot(rf.250ct_2grps, type=1,n.var=10);
varImpPlot(rf.250ct_2grps, type=1,n.var=4);
varImpPlot(rf.250ct_3grps, type=1,n.var=10);
...
# 4.3) Specify the final predictor variables ;
      preds.final=c('Tmean8110Ws','Precip8110Ws','CaOWs','SLOPE','WsAreaSqKm','perStrm','SiO2Ws','PermWs','Fe2O3Ws','ElevCat') # ,'Eco2','Eco3','Eco4_code','East.West'
      
      preds.final=c('Tmean8110Ws','SLOPE','WsAreaSqKm','perStrm') # ,'Eco2','Eco3','Eco4_code','East.West'
...
#detach("package:NAMCr", unload = TRUE)
     save(bugcal.pa, predcal, grps.final, preds.final, rf.250ct_2grps, file=('bugs analyses/RIVPACS_2022/rf.mod.pca_250.Rdata')); # Notice I'm using the most parsimonious model (with 8 predictors)
...
OE.assess.cal<-model.predict.RanFor.4.2(bugcal.pa,grps.final,preds.final, ranfor.mod=rf.250ct_2grps,prednew=predcal,bugnew=bugcal.pa,Pc=0.5, Cal.OOB=TRUE); 
