...
cv <- tss_vars_num %>% 
  ...
  pivot_longer(1:134, names_to="Var", values_to="CV") # Change first argument to 1:# columns

cv_pctzero <- tss_vars_num %>%
  summarise(across(everything(), ~ sum(. == 0) / n() * 100)) %>% 
  pivot_longer(1:134, names_to="Var", values_to="Percent Zeros") %>%  # Change  first argument to 1:# columns
...
#### Select out variables that were not selected based on correlation analysis.
...
hist(tss_vars_selected$logTSS, main = "", xlab = "log TSS") #log-transformed
...
tss_vars_selected %>% 
  ...
  geom_density() +
  geom_rug() + ggtitle("Reference")

# Moderately Disturbed
tss_vars_selected %>% 
  filter(ReferenceSite == 'MODERATELY DISTURBED') %>% 
  select(BANKFULLWIDTH:L3Eco) %>% 
  select('WDRW_LDWS', 'PCTCONIF2001WS' , 'CLAYWS' , 'PESTIC1997WS' , 'SANDWS' , 'PCTCROP2001WS' , 'POPDEN2010WS' , 'PRECIP9120WS' , 'PCTAGSLPMID2001WS' , 'PCT_EROD' , 'SW_FLUXWS' , 'PCTIMP2001WS' , 'RDCRSWS' , 'WTDEPWS' , 'ELEVWS' , 'WETINDEXWS' , 'PCTSHRB2001WS' , 'KFFACTWS' , 'PCTICE2001WS' , 'RCKDEPWS') %>% 
  keep(is.numeric) %>% 
  gather() %>% 
  ...
  facet_wrap(~ key, scales = "free") +
  geom_density() +
  geom_rug() + ggtitle("MODERATELY DISTURBED")
...
tss_vars_selected %>% 
  ...
  facet_wrap(~ key, scales = "free") +
  geom_density() +
  geom_rug() + ggtitle("MOST DISTURBED")
...
partialPlot(rf.red_15, as.data.frame(tss.cal_val), WDRW_LDWS, main = '', ylab = "log Predicted TSS")
abline(v= 0 , col = 'black', lty = 'dashed')
hist(tss.ref.hi$WDRW_LDWS, main = '', xlab='WDRW_LDWS')

# Mean pesticide use kg/sq km
# Want to scale back to 0.
par(mfrow=c(1,2))
partialPlot(rf.red_15, as.data.frame(tss.cal_val), PESTIC1997WS, main = '', ylab = "log Predicted TSS")
abline(v=quantile(tss.ref.hi$PESTIC1997WS, probs = c(0.5)), col = c( 'black'),lty = 'dashed')
hist(tss.ref.hi$PESTIC1997WS, main = '', xlab = 'PESTIC1997WS')
quantile(tss.ref.hi$PESTIC1997WS, probs = seq(.1, .9, by = .1))
...
par(mfrow=c(1,2))
partialPlot(rf.red_15, as.data.frame(tss.cal_val), PCTCROP2001WS, main = '', ylab = "Predicted TSS")
abline(v=quantile(tss.ref.hi$PCTCROP2001WS, probs = c(0.5)), col = c('black'),lty = 'dashed')
hist(tss.ref.hi$PCTCROP2001WS, main = '', xlab = 'PCTCROP2001WS')
quantile(tss.ref.hi$PCTCROP2001WS, probs = seq(.1, .9, by = .1))
...
par(mfrow=c(1,2))
partialPlot(rf.red_15, as.data.frame(tss.cal_val), POPDEN2010WS, main = '', ylab = "log Predicted TSS", scale_x_log10())
abline(v=quantile(tss.ref.hi$POPDEN2010WS, probs = c(0.5)), col = c('black'),lty = 'dashed')
hist(tss.ref.hi$POPDEN2010WS, main = '', xlab = 'POPDEN2010WS')
quantile(tss.ref.hi$POPDEN2010WS, probs = seq(.1, .9, by = .1))
...
par(mfrow=c(1,2))
partialPlot(rf.red_15, as.data.frame(tss.cal_val), PCTAGSLPMID2001WS, main = '', ylab = "log Predicted TSS")
abline(v=quantile(tss.ref.hi$PCTAGSLPMID2001WS, probs = c(0.5)), col = c( 'black'),lty = 'dashed')
hist(tss.ref.hi$PCTAGSLPMID2001WS, main = '', xlab = 'PCTAGSLPMID2001WS')
quantile(tss.ref.hi$PCTAGSLPMID2001WS, probs = seq(.1, .9, by = .1))
...
par(mfrow=c(1,2))
partialPlot(rf.red_15, as.data.frame(tss.cal_val), SW_FLUXWS, main = '', ylab = "log Predicted TSS")
abline(v=quantile(tss.ref.hi$SW_FLUXWS, probs = c(0.5)), col = c('black'),lty = 'dashed')
hist(tss.ref.hi$SW_FLUXWS, main = '', xlab = 'SW_FLUXWS')
quantile(tss.ref.hi$SW_FLUXWS, probs = seq(.1, .9, by = .1))
...
par(mfrow=c(1,2))
partialPlot(rf.red_15, as.data.frame(tss.cal_val), PCTIMP2001WS, main = '', ylab = "log Predicted TSS")
abline(v=quantile(tss.ref.hi$PCTIMP2001WS, probs = c(0.5)), col = c( 'black'),lty = 'dashed')
hist(tss.ref.hi$PCTIMP2001WS, main = '', xlab = 'PCTIMP2001WS')
quantile(tss.ref.hi$PCTIMP2001WS, probs = seq(.1, .9, by = .1))
...
par(mfrow=c(1,2))
partialPlot(rf.red_15, as.data.frame(tss.cal_val), RDCRSWS, main = '', ylab = "log Predicted TSS", xlab = "RDCRSWS")
abline(v = 0, col = 'black',lty = 'dashed')
hist(tss.ref.hi$RDCRSWS, main = '', xlab = 'RDCRSWS', ylim = c(0,100))
abline(v = 0.005, col = 'black',lty = 'dashed')
```

#### Coefficient of Variation for env predictors

```{r}
top20 <- scmets %>% 
select('WDRW_LDWS', 'PCTCONIF2001WS' , 'CLAYWS' , 'PESTIC1997WS' , 'SANDWS' , 'PCTCROP2001WS' , 'POPDEN2010WS' , 'PRECIP9120WS' , 'PCTAGSLPMID2001WS' , 'PCT_EROD' , 'SW_FLUXWS' , 'PCTIMP2001WS' , 'RDCRSWS' , 'WTDEPWS' , 'ELEVWS' , 'WETINDEXWS' , 'PCTSHRB2001WS' , 'KFFACTWS' , 'PCTICE2001WS' , 'RCKDEPWS')

cv1 <- sd(top20$PCTICE2001WS)/mean(top20$PCTICE2001WS)
...
cv_top20<-pivot_longer(cv_top20, 1:20, names_to="Var", values_to="CV")

write_xlsx(cv_top20, path = paste0("C://Users//sberzin//OneDrive - Oregon//Desktop//CofV", Sys.Date(), ".xlsx"))
...
cv_all<-pivot_longer(cv_all, 1:77, names_to="Var", values_to="CV")

write_xlsx(cv_all, path = paste0("C://Users//sberzin//OneDrive - Oregon//Desktop//CofV_all", Sys.Date(), ".xlsx"))
...
```{r}
tss.plots <- tss %>%   # These get removed in the code later, but I'm taking them out here to take advantage of all the columns that came from AWQMs before they get removed.
  filter(MLocID != '23856-ORDEQ') %>% 
  filter(MLocID != '30625-ORDEQ') %>% 
  filter(MLocID != '30727-ORDEQ') %>% 
  filter(MLocID != '12266-ORDEQ')

tss.plots$ReferenceSite <- factor(tss.plots$ReferenceSite, levels = c("REFERENCE", "MODERATELY DISTURBED", "MOST DISTURBED"))

tss.plots.sum <- tss.plots %>% 
...
ggplot(tss.plots, aes(x = ReferenceSite,y = TSS)) +
    geom_boxplot(outlier.size = 0.9) +
    labs(x = "Reference Status", y = "TSS (mg/L)")  +
