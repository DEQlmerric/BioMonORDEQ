...
library(NAMCr)
library(tidyr)
setwd=("C:/Users/jenni/Box/Nonperennial streams/Bugdata/rerun of all models_for pub")
...
predcal=read.csv("C:/Users/jenni/Box/Nonperennial streams/Bugdata/rerun of all models_for pub/metrics_predictors.csv")
#select only reference data and exclude bug metrics
predcal=predcal[predcal$reference=='Y',c(1,97:150)]
rownames(predcal)=predcal$sampleId
#remove sampleId and reference columns
predcal=predcal[,c(-1,-2)]
...
# subset the bug data to only the reference data for now 
bugsOTU=subset(bugsOTU, sampleId %in% rownames(predcal))
...
bugcal.pa = tidyr::pivot_wider(sumrarefiedOTUTaxa,id_cols = "sampleId", names_from = "otuName",values_from = "presence")
# fill in nulls as 0
bugcal.pa[is.na(bugcal.pa)]<-0
bugcal.pa=as.data.frame(bugcal.pa)
rownames(bugcal.pa)<-bugcal.pa$sampleId
# remove sampleID as a column
bugcal.pa<-bugcal.pa[,-1]
...
plot(clus1,which.plots=2,labels=bugcal.pa[,1],cex=.35,main=paste0("Dendrogram, created on ",Sys.time(),", with a horizontal line delineating ", num.clusters, " cluster breaks.")); abline(h=dendrogram.numclust.height[num.clusters],col=4,lty=2);
...
windows()
m=sf::st_as_sf(rfdat, coords = c("LONG", "LAT"), crs = 4269,agr = "constant")
mapview::mapview(m['Groups'])
...
#--------------------------------------------------------------------#
##############
rfdat<-data.frame(predcal,grps.9, grps.11);
rfdat$grps.9<-factor(rfdat$grps.9);
rfdat$grps.11<-factor(rfdat$grps.11)
rfdat$grps.10<-factor(ifelse(rfdat$grps.11=='11','9',rfdat$grps.11))
grps.10=as.integer(rfdat$grps.10)

# Build RF model;
species.vsurf = VSURF(rfdat[,1:53], rfdat[,56])
summary(species.vsurf)

#seeing what the variables are
names = as.data.frame(names(rfdat))
selected.pred=names[species.vsurf$varselect.pred,]
interp.pred=names[species.vsurf$varselect.interp,]
threshold.pred=names[species.vsurf$varselect.thres,]

paste(selected.pred,collapse="+")

#best from vsurf
rf.mod.best.from.VSURF9 = randomForest(grps.9 ~ LONG+ElevCat+MSST_mean08091314+Tmax8110Ws+Precip8110Ws+LAT+WsAreaSqKm+RunoffWs, data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)
rf.mod.best.from.VSURF10 = randomForest(grps.10 ~ LONG+MSST_mean08091314+ElevCat+Tmax8110Ws+Precip8110Ws+length_46006+OmWs+DOY+WsAreaSqKm, data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)
...
windows();
varImpPlot(rf.mod.best.from.VSURF9, type=1,n.var=8);
#varImpPlot(rf.mod, type=1,n.var=48);



# Variable Importance Table
importance(rf.mod.best.from.VSURF10)
# the "type=1" argument below tells it to focus on the 1st metric of importance, the %IncMSE.  
var.imp <- data.frame(importance(rf.mod.best.from.VSURF11, type=1));  var.imp
...
start.time = Sys.time()
op <- par(mfrow=c(3, 3))
for (i in seq_along(imp.names[c(1:9)])) {
  partialPlot(rf.mod.best.from.VSURF11, rfdat, which.class = "9", imp.names[i], xlab=imp.names[i],
...
# 4.3) Specify the final predictor variables ;
      preds.final=c('LONG','ElevCat','MSST_mean08091314','Tmax8110Ws','Precip8110Ws','LAT','WsAreaSqKm','RunoffWs')
...
detach("package:NAMCr", unload = TRUE)
     save(bugcal.pa, predcal, grps.final, preds.final, rf.mod.best.from.VSURF9, file=('rf.mod_9groups.Rdata')); # Notice I'm using the most parsimonious model (with 8 predictors)
...
load("C:/Users/jenni/Box/Nonperennial streams/Bugdata/rerun of all models_for pub/OE/rf.mod_9groups.Rdata")
...
source("C:/Users/jenni/OneDrive - USU/Documents/model.predict.RanFor.4.2.r");
...
OE.assess.cal<-model.predict.RanFor.4.2(bugcal.pa,grps.final,preds.final, ranfor.mod=rf.mod.best.from.VSURF9,prednew=predcal,bugnew=bugcal.pa,Pc=0.5,Cal.OOB=TRUE);
str(OE.assess.cal)
