...
```{r}
#tss <- read_excel("TSS_2025-11-06.xlsx") #output file from ref benchmarks script - no Ambient sites
tss <- read_excel("TSS_2025-11-12amb.xlsx") #output file from ref benchmarks script - Ambient sites included

#tss <- tss %>% filter(TSS < 20) #testing model output by excluding values >20 mg/L
...
rf.full <- randomForest(log(TSS+1) ~ ., tss.cal, importance = TRUE,  mtry = 8, keep.forest = TRUE, ntree = 500) #run random forest model on all predictors
print(rf.full)
...
abline(a=0,b=1)
print(paste("natural log RMSE =", sqrt(sum((log(pred.rf+1) - log(tss.cal$TSS+1))^2)/length(tss.cal$TSS)))) #prediction error
