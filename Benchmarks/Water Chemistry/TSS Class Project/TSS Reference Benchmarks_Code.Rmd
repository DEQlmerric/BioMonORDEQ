...
meow1 <- meow[,-1] # drop COMID
meow1<-scale(meow1[,-10]) #convert each variable to its Z-scores
meow1[is.na(meow1)] <- 0
...
```{r, warning=FALSE}
pca<-princomp(meow1, na.action = na.omit)
...
### **Running a multiple regression model with two PCs**

We can make a new dataset containing our Y (EPTR) and two PCs as Xs

```{r, warning=FALSE}
dta3<-data.frame(EPT=dta$EPTR,PC1=pca$score[,1],PC2=pca$scores[,2])
```

Let's fit a new multiple regression model with two PCs as predictors and then check for multi-collearity

```{r, warning=FALSE}
mod<-lm(EPT~PC1+PC2, data=dta3)
summary(mod)
library(car)
vif(mod)
``
...
write_xlsx(meow, path = paste0("C://Users//sberzin//OneDrive - Oregon//Desktop//sc", Sys.Date(), ".xlsx"))
