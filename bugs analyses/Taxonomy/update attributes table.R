...
# join the tolerance to the OTU table
...
otus_tols <- otus_tols %>%
  left_join(mtti.tols, by = 'OTU_MTTI') %>%
  rename(MTTI_WA.optima = Optima)



otus_tols$BSTI_WA.optima <-  round(otus_tols$BSTI_WA.optima, digits = 1)
otus_tols$MTTI_WA.optima <-  round(otus_tols$MTTI_WA.optima, digits = 1)

otus_tols2 <- otus_tols %>% select(Taxon = Taxon_orig, BSTI_WA.optima, MTTI_WA.optima)
...
attributes.2 <- attributes.2 |>
  relocate(MTTI_WA.optima, .after = ThermTol_Less30) |>
...
write.csv(attributes.2, 'ORWA_Attributes_20260601.csv')
