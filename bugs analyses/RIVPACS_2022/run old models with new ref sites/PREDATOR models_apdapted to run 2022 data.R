...
#         1.20.23 = adapted to run 2022 ref sites thru old models
#         data already aggregated, from outside of the DEQ data systems 
...
bugs <- read.csv('bugs analyses/RIVPACS_2022/run old models with new ref sites/ODEQ22Matrix2005otu.csv')
preds <- read.csv('bugs analyses/RIVPACS_2022/run old models with new ref sites/2005preds_225ref_new sites for running old models.csv')

bugs <- dplyr::rename(bugs, Count = splitCount)
bugs <- dplyr::rename(bugs, OTU = otuName2005)
bugs <- dplyr::rename(bugs, Sample = sampleId)  
  ...
  tot.abund.RIV<-aggregate(bugs$Count, list(Sample=bugs$Sample), sum)
