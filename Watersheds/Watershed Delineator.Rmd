...
![](images/clipboard-1508299075.png)

#### **PREFACE:** This script batch delineates upstream watershed boundaries for a given set of stations/originating COMIDs.
...
#### Choose this option if stations are already in GIS and you don't need COMIDs.

```{r}
sta <- st_read(paste0(input_dir, "/", file_input)) #read GIS shapefile

sta<-sta %>%
  dplyr::mutate(lon = sf::st_coordinates(.)[,1],
                lat = sf::st_coordinates(.)[,2]) #put latitude and longitude into their own columns
```
...
#### Enter the resulting 4-digit coordinate reference system below (in quotes).
...
**6 - *STOP* - REVIEW AUTO-GENERATED COMIDS IN GIS** \## Write code to override any changes. Skip this section if all COMIDs are correct.

```{r}
#input$comid[input$comid=="123456"] <- "456789"
```

**7 - WRITE NEWLY DELINEATED WATERSHEDS TO GIS OUTPUT FILE** \#### This step....
...
**8 - CALCULATE WATERSHED AREAS AND APPEND TO GIS SHAPEFILE** \#### Note: Area is calculated in sq meters. Convert manually if needed.
