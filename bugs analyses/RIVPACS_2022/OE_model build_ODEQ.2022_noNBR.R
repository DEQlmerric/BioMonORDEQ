...
preds=read.csv("bugs analyses/RIVPACS_2022/preds_slope_ecoregion.csv")
...
bugsOTU <- read.csv('bugs analyses/RIVPACS_2022/OR_rare300_225.csv')                        
...
groups9=cbind(row.names(predcal),grps.7); #list calibration sites and their group assignments;
...
groups9=cbind(row.names(predcal),grps.8); #list calibration sites and their group assignments;
...
groups10=cbind(row.names(predcal),grps.9); #list calibration sites and their group assignments;
...
grps.13<-cutree(clus1,k=13); #vector of group assignments is in the order of sites in the clustered data;
table(grps.13); #count number of sites in each group;
groups13=cbind(row.names(predcal),grps.13); #list calibration sites and their group assignments;


grps.15<-cutree(clus1,k=15); #vector of group assignments is in the order of sites in the clustered data;
table(grps.15); #count number of sites in each group;
groups15=cbind(row.names(predcal),grps.15); #list calibration sites and their group assignments;
...
mapview::mapview(m['Groups'],col.regions=brewer.pal(5, "Spectral"))
...
rfdat<-data.frame(predcal,grps.5, grps.7, grps.8, grps.9, grps.10, grps.11);
...
rfdat <- rfdat %>%
  dplyr::select(Tmean8110Ws,Precip8110Ws,CaOWs,SLOPE,WsAreaSqKm,perStrm,SiO2Ws,PermWs,
         Fe2O3Ws,ElevCat,Eco2,Eco3,Eco4_code,East.West, Lab,grps.5, grps.7,grps.8,grps.9,grps.10,grps.11)
...
rfdat$grps.11<-factor(rfdat$grps.11)
...
rf.250ct_5grps = randomForest(grps.5 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat+Eco2+Eco3+Eco4_code+East.West, #,
              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)
                                                                    
                                                                    
rf.250ct_7grps = randomForest(grps.7 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat+Eco2+Eco3+Eco4_code+East.West, #,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)

rf.250ct_7grps = randomForest(grps.7 ~ Tmean8110Ws+Precip8110Ws+WsAreaSqKm+SiO2Ws+Fe2O3Ws+ElevCat+Eco3, #,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)



rf.250ct_8grps = randomForest(grps.8 ~ Tmean8110Ws+Precip8110Ws+Fe2O3Ws+ElevCat+Eco3,#+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)

rf.250ct_9grps = randomForest(grps.9 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat+Eco2+Eco3+Eco4_code+East.West, #+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)

rf.250ct_10grps = randomForest(grps.10 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat+Eco2+Eco3+Eco4_code+East.West, #+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)

rf.250ct_11grps = randomForest(grps.11 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat+Eco2+Eco3+Eco4_code+East.West, #+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)




print(rf.250ct_7grps)

# Variable Importance Plot (VIP):
varImpPlot(rf.250ct_5grps, type=1,n.var=13);
varImpPlot(rf.250ct_7grps, type=1,n.var=7);
varImpPlot(rf.250ct_8grps, type=1,n.var=5);
varImpPlot(rf.250ct_9grps, type=1,n.var=10);
varImpPlot(rf.250ct_10grps, type=1,n.var=10);
varImpPlot(rf.250ct_11grps, type=1,n.var=10);
...
# 4.3) Specify the final predictor variables ;Tmean8110Ws+Precip8110Ws+WsAreaSqKm+SiO2Ws+Fe2O3Ws+ElevCat+Eco3
      preds.final=c('Tmean8110Ws','Precip8110Ws','CaOWs','SLOPE','WsAreaSqKm','perStrm','SiO2Ws','PermWs','Fe2O3Ws','ElevCat','Eco2','Eco3', 'Eco4_code' ) # ,'Eco2','Eco3','Eco4_code','East.West'
      
      preds.final=c('Tmean8110Ws','Precip8110Ws','Fe2O3Ws','ElevCat','Eco3')
...
bugcal.pa = bugcal.pa_250
predcal = rfdat[,c(1:14)]

predcal = rfdat[,c(1,2,9,10,12)]
...
#detach("package:NAMCr", unload = TRUE)
     save(bugcal.pa, predcal, grps.final, preds.final, rf.250ct_8grps, file=('bugs analyses/RIVPACS_2022/rf.mod.pca_250.Rdata')); # Notice I'm using the most parsimonious model (with 8 predictors)
...
load("bugs analyses/RIVPACS_2022/rf.mod.pca_250.Rdata")
...
OE.assess.cal<-model.predict.RanFor.4.2(bugcal.pa,grps.final,preds.final, ranfor.mod=rf.250ct_8grps,prednew=predcal,bugnew=bugcal.pa,Pc=0.5, Cal.OOB=TRUE); 
rep.sam.sd(occprb.cal=OE.assess.cal$Capture.Probs,Pc=0.5);



            OE.assess.cal<-model.predict.RanFor(bugcal.pa,grps.final,preds.final, ranfor.mod=rf.250ct_8grps,prednew=predcal,bugnew=bugcal.pa,Pc=0.5); # , Cal.OOB=TRUE
            rep.sam.sd(occprb.cal=OE.assess.cal$Capture.Probs,Pc=0.5);
