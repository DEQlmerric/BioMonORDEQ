...
cv <- tss_vars_num %>% 
  ...
  pivot_longer(1:137, names_to="Var", values_to="CV") # Change first argument to 1:# columns

cv_pctzero <- tss_vars_num %>%
  summarise(across(everything(), ~ sum(. == 0) / n() * 100)) %>% 
  pivot_longer(1:137, names_to="Var", values_to="Percent Zeros") %>%  # Change  first argument to 1:# columns
