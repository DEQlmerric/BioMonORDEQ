...
import <- stations |>
  filter(MLocID %in% c("21848-ORDEQ","10404-ORDEQ")) #<--Enter desired MLocIDs in quotes separated by commas
```
...
start_comid <- "23648048" #option for delineating a single COMID
...
st_write(catchment, save_file, append = TRUE, message("Writing watersheds to specified output file"))
}
```
...
```{r}
mapview(catchment, col.regions = "palegreen", layer.name = "Watershed", alpha.regions = 0.5, legend = FALSE) + 
