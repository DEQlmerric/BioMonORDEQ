...
sapply(tp_vars, function(x) sum(is.na(x)))

# PERFORM NA ROUGHFIX FUNCTION ON METRICS WITH MISSING VALUES
tp_vars$NSURPWS  <- na.roughfix(tp_vars$NSURPWS) # 1 NA
tp_vars$NANIWS  <- na.roughfix(tp_vars$NANIWS) # 1 NA
tp_vars$SW_FLUXWS  <- na.roughfix(tp_vars$SW_FLUXWS) # 2 NA
tp_vars$PCTIMPSLPHIGH2001WS  <- na.roughfix(tp_vars$PCTIMPSLPHIGH2001WS) # 4 NA
tp_vars$MAST2008  <- na.roughfix(tp_vars$MAST2008) # 56 NA
tp_vars$MWST2008  <- na.roughfix(tp_vars$MWST2008) # 9 NA
tp_vars$PRG_BMMI0809  <- na.roughfix(tp_vars$PRG_BMMI0809) # 62 NA
tp_vars$WETTEDWIDTH  <- na.roughfix(tp_vars$WETTEDWIDTH) # 64 NA
tp_vars$THALWEGDEPTH  <- na.roughfix(tp_vars$THALWEGDEPTH) # 64 NA
tp_vars$BANKFULLWIDTH  <- na.roughfix(tp_vars$BANKFULLWIDTH) # 64 NA
tp_vars$BANKFULLDEPTH  <- na.roughfix(tp_vars$BANKFULLDEPTH) # 64 NA
...
spear <- cor(tp_vars_num, method = "spearman", use = "pairwise.complete.obs")
...
write_xlsx(matrix, path = "~/BioMonORDEQ/Benchmarks/Water Chemistry/Total Phosphorus/TP_Spearmans_Matrix.xlsx")
