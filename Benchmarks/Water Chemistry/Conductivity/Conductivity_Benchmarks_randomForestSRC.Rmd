...
m <- as.data.frame(correlations)

# Resids of higher urban pop only 
cond_map_urb <- cond_map %>%
  filter(PCTURBLO2001WS > 1)

ggplot(data=cond_map_urb, aes(x = Long_DD, y = delta)) + geom_point() + geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Longitude", y = "Obs - Pred")

map_function(cond_map_urb)
...
rf.full.no_hi <- rfsrc(logCond ~ ., data = cond.src.no_hi, mtry = 57, nodesize = 2, importance = TRUE)
...
predicted.oob.no_hi <- data.frame(obs = cond.src.no_hi$logCond, pred = rf.full.no_hi$predicted.oob, predictions = "OOB") # The prediction is computed using only the trees in which that specific case was not used for training.
predicted.ib.no_hi <- data.frame(obs =cond.src.no_hi$logCond, pred = rf.full.no_hi$predicted, predictions = "In-Bag") # This contains the predicted values for each case in the training data based on the entire ensemble (all trees). It uses the data that was "in-bag" (used to grow the trees) to determine the terminal node predictions.


# Plot residuals - full model, adjusted human-influenced vars
residuals.rf.full.no_hi <- exp(cond.src.no_hi$logCond) - exp(predicted.oob.no_hi$pred) # Observed minus predicted oob
...
plot.variable(rf.full.no_hi, xvar.names = c("PRECIP9120WS", "KFFACTWS", "TMAX9120WS", "OMWS", "L3ECO", "PCTBL2001WS", "ELEVWS", "WTDEPWS", "PCTDECID2001WS", "SWS", "PCTOW2001WS", "PCT_EROD"), partial = TRUE, plot.sone.page = TRUE)
```

#### Temporary code to explore map after adjusting human vbls to 0
...
# Join predictions to cal_val table
cond.obs_predict <- cbind(plotdf, cond.src.no_hi)
