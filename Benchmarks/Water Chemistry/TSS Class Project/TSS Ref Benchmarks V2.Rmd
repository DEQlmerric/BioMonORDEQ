...
tss_vars <- tss_vars %>% 
  ...
  relocate(Lat_DD:Long_DD, .after = NHD_pSLOPE)
```
...
tss_vars_num <- tss_vars %>% 
  select(PCTAGDRAINAGEWS:Long_DD)
...
cv <- tss_vars_num %>% 
  ...
  pivot_longer(1:139, names_to="Var", values_to="CV")

cv_pctzero <- tss_vars_num %>%
  summarise(across(everything(), ~ sum(. == 0) / n() * 100)) %>% 
  pivot_longer(1:139, names_to="Var", values_to="Percent Zeros") %>% 
...
#### Drop variables that were not selected based on correlation analysis.
```{r}
tss_vars_selected <- tss_vars %>% 
  select(!c(NSURPWS,NANIWS,PCTAGSLPHIGH2001WS,WATERINPUTWS,AGKFFACTWS,CBNFWS,MANUREWS,FERTWS,PCTHAY2001WS,
            TMAX8110WS,PRECIP8110WS,NO32008WS,TMEAN8110WS,PRECIP2008WS,INORGNWETDEP2008WS,NH42008WS,TMIN8110WS,
            TMEAN2008WS,SN2008WS,TMAX9120WS,TMEAN9120WS,TMIN9120WS,NABD_DENSWS,DAMNRMSTORWS,NABD_NRMSTORWS,
            NABD_NIDSTORWS,DAMNIDSTORWS,RUNOFFWS,HABTWS,HYDWS,CONNWS,TEMPWS,CHEMWS,SEDWS,PCTURBLO2001WS,
            PCTDECID2001WS,SIO2WS,K2OWS,P2O5WS,AL2O3WS,CAOWS,MGOWS,PERMWS,PCTSILICICWS,PCTNONCARBRESIDWS,
            WWTPMINORDENSWS,PCTIMPSLPHIGH2001WS,PCTIMPSLPMID2001WS,HUDEN2010WS,PCTBURNAREA2002WS,
            PCTUNDSEV2002WS,PCTLOWSEV2002WS,PCTMODSEV2002WS,PCTHIGHSEV2002WS,ICI,IWI,MAST2008,MWST2008,
            WETTEDWIDTH,THALWAGDEPTH,BANKFULLDEPTH))
```
...
```{r}
join <- inner_join(scmets, tss, by='COMID')

join_ref_plot <- join %>% 
  filter(ReferenceSite == 'REFERENCE') %>% 
  select(SW_FLUXWS:NHD_pSLOPE) 
...
join_ND_plot %>%
  ...
  facet_wrap(~ key, scales = "free") +
  geom_density() +
  geom_rug() + ggtitle("Non-detects")

join_midTSS_plot <- join %>% 
