...
if(type == "IWI"){
  ...
  streamcat <- purrr::map_dfr(comids, ~StreamCatTools::sc_get_data(.,
                                                                           metric = 'chem,conn,habt,hyd,sed,temp,prg_bmmi0809', 
                                                                           showAreaSqKm = FALSE,
                                                                           aoi='watershed'))
  }
...
if(type == "Others"){
  ...
  streamcat <- purrr::map_dfr(comids, ~StreamCatTools::sc_get_data(.,
                                                                           metric = 'bankfulldepth,bankfullwidth,ici,iwi,mast2008,msst2008,mwst2008,thalwegdepth,wettedwidth', 
                                                                           showAreaSqKm = FALSE,
                                                                           aoi='other')) 
  }
...
cond_vars_num$CLAYWS  <- na.roughfix(cond_vars_num$CLAYWS ) # 1 NA
cond_vars_num$MSST2008 <- na.roughfix(cond_vars_num$ MSST2008) # 1 NA
cond_vars_num$MWST2008 <- na.roughfix(cond_vars_num$ MWST2008) # 12 NAs
cond_vars_num$NANIWS <- na.roughfix(cond_vars_num$NANIWS) # 1 NA
cond_vars_num$NSURPWS <- na.roughfix(cond_vars_num$NSURPWS) # 1 NA
cond_vars_num$OMWS <- na.roughfix(cond_vars_num$OMWS) # 1 NA
cond_vars_num$PCTIMPSLPHIGH2001WS <- na.roughfix(cond_vars_num$PCTIMPSLPHIGH2001WS) # 4 NAs
cond_vars_num$PERMWS <- na.roughfix(cond_vars_num$PERMWS) # 1 NA
cond_vars_num$RCKDEPWS <- na.roughfix(cond_vars_num$RCKDEPWS) # 1 NA
cond_vars_num$SANDWS <- na.roughfix(cond_vars_num$SANDWS) # 1 NA
cond_vars_num$SW_FLUXWS <- na.roughfix(cond_vars_num$SW_FLUXWS) # 1 NA
cond_vars_num$WTDEPWS <- na.roughfix(cond_vars_num$WTDEPWS) # 1 NA
...
#write_xlsx(spear.rows, path = "~/BioMonORDEQ/Benchmarks/Water Chemistry/Conductivity/Cond_Spearmans_Full_Matrix.xlsx")
...
##### NOTE: Take the correlation matrix and make predictor decisions. Look at CV and proportion of 0s to help make these decisions.
...
kd_plots <- function(data, filter_col_name, filter_value) {
  data %>%
    ...
    select("AGKFFACTWS","AL2O3WS","BANKFULLDEPTH","BANKFULLWIDTH","BFIWS","CAOWS","CBNFWS","CLAYWS","CONNWS","DAMDENSWS","DAMNIDSTORWS","DAMNRMSTORWS") %>%  
  keep(is.numeric) %>% 
  gather() %>% 
  ggplot(aes(value)) +
...
# broken out by reference status
kd_plots(data = cond_vars, "ReferenceSite", "REFERENCE")
kd_plots(data = cond_vars, "ReferenceSite", "MODERATELY DISTURBED")
kd_plots(data = cond_vars, "ReferenceSite", "MOST DISTURBED")

# broken out by L2 Ecoregion
kd_plots(data = cond_vars, "L2Eco", "Western Cordillera")
kd_plots(data = cond_vars, "L2Eco", "Cold Deserts")
kd_plots(data = cond_vars, "L2Eco", "Marine West Coast Forest")
...
```
 ...
 # Not edited beyond this point!  SYB 1/7/26
...
#### Select variables based on correlation analysis. (Either select desired vars or drop undesired ones. Helps if they are in alphabetical order. Don't get rid of columns you want to keep from AWQMS at this stage.)

```{r}
tss_vars_selected <- tss_vars %>% 
  select(c(MLocID:COMID,BANKFULLWIDTH,BFIWS,CANALDENSWS,CLAYWS,COALMINEDENSWS,COMPSTRGTHWS,DAMDENSWS,ELEVWS,FE2O3WS,HYDRLCONDWS,KFFACTWS,MINEDENSWS,MSST2008,NWS,NA2OWS,NPDESDENSWS,OMWS,PCTAGDRAINAGEWS,PCTAGSLPMID2001WS,PCTALKINTRUVOLWS,PCTALLUVCOASTWS,PCTBL2001WS,PCTCARBRESIDWS,PCTCOASTCRSWS,PCTCOLLUVSEDWS,PCTCONIF2001WS,PCTCROP2001WS,PCTEOLCRSWS,PCTEOLFINEWS,PCTEXTRUVOLWS,PCTFIRE2002WS,PCTFRSTLOSS2002WS,PCTGLACLAKECRSWS,PCTGLACLAKEFINEWS,PCTGLACTILCLAYWS,PCTGLACTILCRSWS,PCTGLACTILLOAMWS,PCTGRS2001WS,PCTHBWET2001WS,PCTHYDRICWS,PCTICE2001WS,PCTIMP2001WS,PCTINCVEGRESP2002WS,PCTMXFST2001WS,PCTNONAGINTRODMANAGVEGWS,PCTNONPROCMASK2002WS,PCTOW2001WS,PCTSALLAKEWS,PCTSHRB2001WS,PCTURBHI2001WS,PCTURBMD2001WS,PCTURBOP2001WS,PCTWATERWS,PCTWDWET2001WS,PESTIC1997WS,POPDEN2010WS,PRECIP9120WS,RCKDEPWS,RDCRSWS,RDCRSSLPWTDWS,RDDENSWS,ROCKNWS,SWS,SANDWS,SEPTICWS,SUPERFUNDDENSWS,SW_FLUXWS,TRIDENSWS,WDRW_LDWS,WETINDEXWS,WTDEPWS,WWTPALLDENSWS,WWTPMAJORDENSWS, PCT_EROD:L3Eco)) # 79 vars in total starting with BANKFULLWIDTH
```
...
tss_vars_selected$BANKFULLWIDTH <- na.roughfix(tss_vars_selected$BANKFULLWIDTH) #bankfull width (n = 66 NAs)
tss_vars_selected$SW_FLUXWS <- na.roughfix(tss_vars_selected$SW_FLUXWS) #surface water flux (n = 2 NAs)
...
```{r}
# HISTOGRAM
hist(cond_vars$Conductivity, main = "", xlab = "Conductivity") #raw data

cond_vars <- cond_vars %>% 
  mutate(logCond = log(Conductivity)) %>% 
  relocate(.after= Conductivity)

hist(cond_vars$logCond, main = "", xlab = "log(Conductivity)")

# DENSITY PLOT
hist(tss_vars_selected$logTSS, main="", prob=T, xlab = "TSS")
d <- density(tss_vars_selected$logTSS) #Estimate probability density and save it into **d**
...
kd_plots <- function(data, filter_col_name, filter_value) {
  data %>%
    filter(.data[[filter_col_name]] == filter_value) %>% # Using .data[[]] for dynamic column names
    select(BANKFULLWIDTH:L3Eco) %>% 
    select('WDRW_LDWS', 'PCTCONIF2001WS' , 'CLAYWS' , 'PESTIC1997WS' , 'SANDWS' , 'PCTCROP2001WS' , 'POPDEN2010WS' , 'PRECIP9120WS' , 'PCTAGSLPMID2001WS' , 'PCT_EROD' , 'SW_FLUXWS' , 'PCTIMP2001WS' , 'RDCRSWS' , 'WTDEPWS' , 'ELEVWS' , 'WETINDEXWS' , 'PCTSHRB2001WS' , 'KFFACTWS' , 'PCTICE2001WS' , 'RCKDEPWS') %>% 
  keep(is.numeric) %>% 
  gather() %>% 
  ggplot(aes(value)) +
  facet_wrap(~ key, scales = "free") +
...
kd_plots(data = tss_vars_selected, "ReferenceSite", "REFERENCE")
kd_plots(data = tss_vars_selected, "ReferenceSite", "MODERATELY DISTURBED")
kd_plots(data = tss_vars_selected, "ReferenceSite", "MOST DISTURBED")

# Top 20 variables from RF model broken out by L2 Ecoregion
kd_plots(data = tss_vars_selected, "L2Eco", "Western Cordillera")
kd_plots(data = tss_vars_selected, "L2Eco", "Cold Deserts")
kd_plots(data = tss_vars_selected, "L2Eco", "Marine West Coast Forest")

# Top 20 variables from RF model broken out by TSS category (ND, med, high)
# Non-detects
tss_vars_selected %>%
  filter(TSS<=0.25 | TSS<=0.5) %>%  # N = 356
  select(BANKFULLWIDTH:L3Eco) %>% 
  select('WDRW_LDWS', 'PCTCONIF2001WS' , 'CLAYWS' , 'PESTIC1997WS' , 'SANDWS' , 'PCTCROP2001WS' , 'POPDEN2010WS' , 'PRECIP9120WS' , 'PCTAGSLPMID2001WS' , 'PCT_EROD' , 'SW_FLUXWS' , 'PCTIMP2001WS' , 'RDCRSWS' , 'WTDEPWS' , 'ELEVWS' , 'WETINDEXWS' , 'PCTSHRB2001WS' , 'KFFACTWS' , 'PCTICE2001WS' , 'RCKDEPWS') %>% 
  keep(is.numeric) %>% 
  gather() %>% 
...
tss_vars_selected %>%
  ...
  facet_wrap(~ key, scales = "free") +
  geom_density() +
  geom_rug() + ggtitle("TSS 1-20 mg/L")
...
write_xlsx(tss.cal_val, path = paste0("C://Users//sberzin//OneDrive - Oregon//Desktop//TSS-Cal-val_New", Sys.Date(), ".xlsx"))
```
