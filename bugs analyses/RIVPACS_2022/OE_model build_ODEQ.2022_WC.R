...
rf.250ct_7grps = randomForest(grps.7 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat+Eco4_code+East.West, #,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)


rf.250ct_7grps = randomForest(grps.7 ~ Tmean8110Ws+ElevCat+Eco3, #,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)


rf.250ct_8grps = randomForest(grps.8 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat+Eco3+East.West, #+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)
...
varImpPlot(rf.250ct_7grps, type=1,n.var=10);
varImpPlot(rf.250ct_7grps, type=1,n.var=4);
...
# 4.3) Specify the final predictor variables ;
      preds.final=c('Tmean8110Ws','Precip8110Ws','CaOWs','SLOPE','WsAreaSqKm','perStrm','SiO2Ws','PermWs','Fe2O3Ws','ElevCat', 'Eco3','East.West') # ,'Eco2','Eco3','Eco4_code','East.West'
      
      preds.final=c('Tmean8110Ws','Eco4_code','ElevCat') # ,'Eco2','Eco3','Eco4_code','East.West'
...
#detach("package:NAMCr", unload = TRUE)
     save(bugcal.pa, predcal, grps.final, preds.final, rf.250ct_7grps, file=('bugs analyses/RIVPACS_2022/rf.mod.pca_250.Rdata')); # Notice I'm using the most parsimonious model (with 8 predictors)
...
OE.assess.cal<-model.predict.RanFor.4.2(bugcal.pa,grps.final,preds.final, ranfor.mod=rf.250ct_7grps,prednew=predcal,bugnew=bugcal.pa,Pc=0.5, Cal.OOB=TRUE); 
