...
#### Add additional variables not from SC (Erodible vs. resistant, slope, stream power)
...
# Will remove PCT_SILICICWS and PCT_NONCARBRESIDWS since the are correlated with PCT_EROD >0.8
...
```{r}
# HISTOGRAM
hist(log(tss$TSS), main = "", xlab = "TSS")
...
# FULL MODEL
set.seed(123)
rf.full <- randomForest(log(TSS) ~ ., tss.cal, importance = TRUE,  keep.forest = TRUE, ntree = 500) #run random forest model on all predictors
print(rf.full)

varImpPlot(rf.full, type=1, n.var = 20) #variable importance plot
vip(rf.full, aesthetics = list(color = "blue", fill = "blue")) #another plot

# reprtree:::plot.reprtree(ReprTree(rf.full, tss.cal, metric='d2'),depth=6)
# reprtree:::plot.getTree(rf.full, k=1)
...
plot(pred.rf ~ log(tss.cal$TSS), xlab="Observed TSS", ylab="Predicted TSS")
abline(a=0,b=1)
...
rf.red <- randomForest(TSS ~ POPDEN2010WS + PCTAGSLPMID2001WS + WDRW_LDWS + PESTIC1997WS + ELEVWS + SWS + 
          PCTCONIF2001WS + ROCKNWS + RDCRSWS + SANDWS + CLAYWS, data = tss.cal, importance = FALSE,  keep.forest = FALSE, ntree = 500) 
print(rf.red)
...
#### Run Ref Only model with test data
