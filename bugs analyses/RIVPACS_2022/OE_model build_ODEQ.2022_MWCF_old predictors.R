...
rfdat <- rfdat %>%
  dplyr::select(daynum, long,  grps.2)#,Eco4_code,East.West,Lab,grps.3,grps.9,grps.10,grps.11
...
rf.250ct_2grps = randomForest(grps.2 ~ daynum+long, #,
...
varImpPlot(rf.250ct_2grps, type=1,n.var=2);
...
#detach("package:NAMCr", unload = TRUE)
     save(bugcal.pa, predcal, grps.final, preds.final, rf.250ct_2grps, file=('bugs analyses/RIVPACS_2022/MWCF_rf.mod_05preds_200.Rdata')); # Notice I'm using the most parsimonious model (with 8 predictors)
...
load("bugs analyses/RIVPACS_2022/MWCF_rf.mod_05preds_200.Rdata")
