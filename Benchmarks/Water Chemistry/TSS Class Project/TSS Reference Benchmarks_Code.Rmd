...
```{r}
pred.rf.red_15.val <- predict(rf.red_15, tss.val) #model prediction on 'VAL' dataset
plot(tss.val$TSS ~ pred.rf.red_15.val, xlab = "Observed TSS", ylab = "Predicted TSS", log = "xy")
abline(a=0,b=1)
(rmse<-sqrt(sum((log(pred.rf.red_15.val+1) - log(tss.cal$TSS+1))^2)/length(tss.cal$TSS))) #prediction error
print(paste("inverse natural log RMSE =", exp(rmse)-1))
```


```{r}
pred.rf.red_15.val <- predict(rf.red_15, tss.val.refadj) #model prediction on 'VAL' dataset bringing human-influenced vars to reference levels
plot(tss.val$TSS ~ pred.rf.red_15.val, xlab = "Observed TSS", ylab = "Predicted TSS", log = "xy")
abline(a=0,b=1)
(rmse<-sqrt(sum((log(pred.rf.red_15.val+1) - log(tss.cal$TSS+1))^2)/length(tss.cal$TSS))) #prediction error
print(paste("inverse natural log RMSE =", exp(rmse)-1))
