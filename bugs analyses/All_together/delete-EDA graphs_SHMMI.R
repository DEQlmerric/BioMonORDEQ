...
SH_MMI <- read.csv('C:/Users/tpritch/OneDrive - Oregon/R Projects/BioMonORDEQ/bugs analyses/MMI/_2024 model build/final_MMI_4.metrics.csv') |> 
...
p1 <- ggplot(data = joined_OE_BCG_MMI, aes(x = Continuous_BCG_Level, y = OoverE))+
  ...
  geom_vline(xintercept = 3.5, color = "purple")+
...
p2 <- ggplot(data = joined_OE_BCG_MMI, aes(x = MMI, y = OoverE))+
  ...
  #stat_smooth(method=lm)+
  geom_hline(yintercept = 0.8, color = 'red')+
  geom_vline(xintercept = 0.55, color = "forestgreen")+
  geom_smooth(#data = filter(joined_OE_BCG_MMI, ReferenceSite == 'REFERENCE' ), 
    method='lm')+
  scale_color_hue(l=50)+
  labs(title = 'O:E + MMI',
       subtitle = "SH generated MMI")+
  theme_bw()
...
p3 <- ggplot(data = joined_OE_BCG_MMI, aes(x = Continuous_BCG_Level, y = MMI))+
  geom_point(aes(color = ReferenceSite))+
  #stat_smooth(method=lm)+
  geom_hline(yintercept = 0.55, color = 'forestgreen')+
  geom_vline(xintercept = 3.5, color = "purple")+
  geom_smooth(#data = filter(joined_OE_BCG_MMI, ReferenceSite == 'REFERENCE' ), 
    method='lm', formula= y~x)+
  scale_color_hue(l=50)+
