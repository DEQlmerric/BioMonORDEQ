...
```{r}
export <- "H:/Personal_Working_Folders/AThompson/WatershedToolComparison2026/NewToolOutput/test2.shp"
...
```{r}
import <- query_stations(mlocs = c("103KLCMSR","103PS0281","23718703","23751778","23718705")) #<--Enter desired MLocIDs in quotes separated by commas
```
...
dataframe_comid <- import |>
  select(COMID, MLocID, StationDes, QC_Comm) |>
...
## Option 1 Sabine's code chunk - run to view delineations in R Viewer. Doesn't currently write to shpfile.
```{r}
# RUN FUNCTION
delin <- function(comid, MLocID, long, lat){
...
station <- st_sfc(st_point(c(long,lat)), crs = 4269) # NAD1983
station <- st_transform(station, 2992) # Oregon Albers
...
catchment2$Shape_Area <- st_area(catchment2) * 0.092903 # Square ft (Oregon Lambert) to square meters

# # APPEND WATERSHEDS TO SHAPEFILE
# st_write(catchment2, save_file, append = TRUE)

mapview(flowlines) + mapview(catchment2) + mapview(station, cex=4)
}

purrr::pmap(dataframe_comid, ~delin(comid = {..1}, MLocID = {..2}, long = {..5}, lat = {..4}))

```

# Option 2 Adam's code chunk - write delineations to shapefile without viewing in R
```{r}
# RUN FUNCTION
