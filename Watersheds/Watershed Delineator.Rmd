...
#### Use *ONE* of the following data import options below (GIS shapefile, Excel, or CSV)
...
```{r}
analyst <- "Dr Stations" #enter your name in quotes
...
#### Testing putting this up sooner.
```{r}
...
#### *Required*: Look up the coordinate reference system (CRS) for your set of stations (see last line of resulting code).
```{r}
st_crs(sta)
```
...
sta<-sta %>%
  ...
  mutate(comid2 = generatecomids(long, lat, refsystem = crs)) #put lat/longs into their own columns & generate comids based on lat/longs
```
...
# **3 - RUN FUNCTION TO DELINEATE WATERSHEDS**
```{r}
...
**4 - *STOP* - REVIEW AUTO-GENERATED COMIDS IN GIS**
...
**5 - WRITE NEWLY DELINEATED WATERSHEDS TO GIS OUTPUT FILE**
#### This step....
```{r}
comid <- data.frame(comid = sta$ComID)

#version 1
purrr::map(comids, ~ delineate_comid_watersheds(.,shp_file = paste0(input_dir, "/", file_input))
...
**6 - CALCULATE WATERSHED AREAS AND APPEND TO GIS SHAPEFILE**
