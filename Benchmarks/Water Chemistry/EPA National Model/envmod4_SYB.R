...
envmod <- function(ss0, runmod = F, doimport = F, doenvplots = F) {
  ...
  envdata.pred <- rbind(ss0[[1]] %>% select(-c(ABISKOMYIA:ZUMATRICHIA)), ss0[[2]] %>% select(-c(ABISKOMYIA:ZUMATRICHIA)))
  ...
  svar <- c("turb.result", "anc.result", "chloride.result", "sulfate.result")
  svarlab <- c("Turbidity", "ANC", "Cl", "SO4")
