...
rfdat<-data.frame(predcal,Groups=grps.7);
windows()
m=sf::st_as_sf(rfdat, coords = c("longitude", "latitude"), crs = 4269,agr = "constant")
mapview::mapview(m['Groups'],col.regions=brewer.pal(7, 'Spectral'), label=TRUE, use.layer.names=TRUE)
...
rfdat <- rfdat %>%
  dplyr::select(Tmean8110Ws,Precip8110Ws,CaOWs,SLOPE,WsAreaSqKm,perStrm,SiO2Ws,PermWs,
         Fe2O3Ws,ElevCat,Eco2,Eco3,Eco4_code,East.West,Lab, grps.7,grps.8)#,grps.9,grps.10,grps.11
...
rf.250ct_7grps = randomForest(grps.7 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat+Eco2+Eco3+East.West+Eco4_code+Lab, #,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)
rf.250ct_8grps = randomForest(grps.8 ~ Tmean8110Ws+Precip8110Ws+CaOWs+SLOPE+WsAreaSqKm+perStrm+SiO2Ws+PermWs+Fe2O3Ws+ElevCat+Eco2+Eco3+East.West+Eco4_code+Lab, #+Eco2+Eco3+Eco4_code+East.West,
                              data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE)
...
varImpPlot(rf.250ct_8grps, type=1,n.var=10);
...
# 4.3) Specify the final predictor variables ;
      preds.final=c('Tmean8110Ws','Precip8110Ws','CaOWs','SLOPE','WsAreaSqKm','perStrm','SiO2Ws','PermWs','Fe2O3Ws','ElevCat+Eco2','Eco3', 'Eco2', 'Eco4_code', 'Lab') # ,'Eco2','Eco3','Eco4_code','East.West'
