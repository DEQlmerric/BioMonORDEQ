...
tss.cal <- tss.sc %>% 
  filter(cal_val == 'CAL') %>%  
  #select(-c(MLocID:COMID, PCT_EROD:SITE_TYPE)) %>% 
...
#### Full model - all predictors included (n = 85)

```{r}
# FULL MODEL
set.seed(42)

rf.full <- randomForest(TSS ~ ., tss.cal, importance = FALSE,  keep.forest = TRUE, ntree = 500) #run random forest model on all predictors
print(rf.full)
