...
rfdat<-data.frame(predcal,grps.7); #, grps.9, grps.10, grps.11


rfdat <- rfdat %>%
  dplyr::select(elev_sqrt, precip, temp, east, grps.7)#,grps.9,grps.10,grps.11
...
rf.250ct_7grps = randomForest(grps.7 ~ elev_sqrt + precip + temp + east,
...
# 4.3) Specify the final predictor variables ;
      preds.final=c('elev_sqrt', 'precip', 'temp', 'east')
...
#detach("package:NAMCr", unload = TRUE)
     save(bugcal.pa, predcal, grps.final, preds.final, rf.250ct_7grps, file=('bugs analyses/RIVPACS_2022/WCCP_rf.mod_05preds_250.Rdata')); # Notice I'm using the most parsimonious model (with 8 predictors)
...
load("bugs analyses/RIVPACS_2022/WCCP_rf.mod_05preds_250.Rdata")
