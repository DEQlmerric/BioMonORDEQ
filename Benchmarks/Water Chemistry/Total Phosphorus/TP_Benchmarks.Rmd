...
```{r}
# 1. Make a variable selection decisions sheet in Excel based on the TEMPLATE (Benchmarks >> Water Chemistry) and save to model folder on GitHub.

# 2. Clean up correlation matrix Excel table manually:
  # a) Filter data to hide 1s.
  # b) Sort row names alphabetically.
  # c) Remove composite indexes (CHEM, CONN, HABT, HYD, ICI, IWI, SED, and TEMP).
  # d) Make a new matrix predictor decisions sheet based on the TEMPLATE (Benchmarks >> Water Chemistry) and save to model folder on GitHub. 
  #     Copy row names into template (col A) and update formulas, column names into col K, and correlations into col K. 
...
# 4. Record which predictors to keep/remove and document rationale.
...
# Use Excel formula (=TEXTJOIN(", ", TRUE, range:range)) to compile list and paste below.
...
```{r}
# Predictions
pred.rf.red_8_val <- predict(rf.red_8, newdata = tp.val)

# Model error 
red_8_val.rmse<- rmse(pred.rf.red_8_val, tp.val$TP)
red_8_val.rmse_scaled <- red_8_val.rmse/range_tp

# Plot
plot(tp.val$TP ~ pred.rf.red_8_val, xlab = 'predicted Total Phosphorus (mg/L)', ylab = 'observed Total Phosphorus (mg/L)', main = 'VALIDATION Reduced Model Top 8')
abline(a=0,b=1)
text(0.1,0.5, paste0("RMSE = ",round(red_8_val.rmse, 2)," | scaled RMSE = ", round(red_8_val.rmse_scaled*100,2), "%"))
```
...
# Run reference only model only for natural predictors
rf.ref.full <- randomForest(TP ~ BFIWS+CLAYWS+COMPSTRGTHWS+ELEVWS+FE2O3WS+HYDRLCONDWS+K2OWS+KFFACTWS+MSST2008+NHD_pSLOPE+NWS+OMWS+P2O5WS+PCT_EROD+PCTALKINTRUVOLWS+
                              _MINUS_EVTWS+RCKDEPWS+ROCKNWS+SANDWS+STRMPOW_CAT+
                              SWS+TMAX9120WS+WETINDEXWS+WTDEPWS+SITE_TYPE+L2Eco+L3Eco, data = tp.ref, importance = TRUE, mtry = 37, keep.forest = TRUE, ntree = 500) #run random forest model on all predictors. mtry=37 based on Caret grid search results.
print(rf.ref.full)
